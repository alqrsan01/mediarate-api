...
// Monthly breakdown
$stmt2 = $pdo->prepare("
    SELECT EXTRACT(MONTH FROM updated_at) AS month, media_type, COUNT(*) AS count
    FROM user_media
    WHERE user_id = ? AND status = 'watched' AND EXTRACT(YEAR FROM updated_at) = ?
    GROUP BY EXTRACT(MONTH FROM updated_at), media_type
    ORDER BY month
");
